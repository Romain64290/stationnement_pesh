...
/**************************************************************************
 Ajout d'un administrateur ou Assistant de Prevention
***************************************************************************/
 
 
 function addRegister($membre,$service,$nom,$prenom,$email,$repassword)
  {
...
if($quantite==0){
		...
		try {
$insert = $this->con->prepare('INSERT INTO membres (id_typemembre,mot_de_passe,nom_membre,prenom_membre,email,date_inscription,service)
VALUES(:id_typemembre,:mot_de_passe,:nom,:prenom,:email,:date,:service)');
 	
$insert->bindParam(':id_typemembre', $membre, PDO::PARAM_INT);
...
// Sujet du mail
$mail->Subject = "Système de contrôle automatisé de stationnement - Modification de mot de passe";
// Le message
$mail->MsgHTML($body);
...
$update->execute();	
  ...
  // calcul le nombre de ligne mise à jour : si incohérence entre id / token / email => count =0
